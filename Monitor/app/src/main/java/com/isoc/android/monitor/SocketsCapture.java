...
/**
 * Captures active sockets by reading files tcp tcp6 udp udp6 raw raw6 stored at
 * /net/proc/ BUG: Shared uids produce uncertainty over which app initiated the
 * connection.
 *  It's only vendor / system apps though that share uids...
 */
public class SocketsCapture {
    ...
    private static String Connection4 = "\\d+: "
            + socket4
            + " "
            + socket4
            + " \\p{XDigit}{2} \\p{XDigit}*:\\p{XDigit}* \\p{XDigit}{2}:\\p{XDigit}{8} \\p{XDigit}{8}\\s+\\d+";
    private static String Connection6 = "\\d+: "
            + socket6
            + " "
            + socket6
            + " \\p{XDigit}{2} \\p{XDigit}*:\\p{XDigit}* \\p{XDigit}{2}:\\p{XDigit}{8} \\p{XDigit}{8}\\s+\\d+";
    ...
    public static void getSockets(SQLiteDatabase db) {
        ...
        // list of the names of the interesting files
        String[] types = { "tcp", "udp", "raw", "tcp6", "udp6", "raw6" }; 
        for (String type : types) {
            String content = NetworkCapture.readStatsFromFile("/proc/net/"
                    + type);
            Matcher m;
            boolean is6 = type.contains("6");
            // if it's an ipv6 file, use socket6 pattern
            if (is6)
                m = Pattern.compile(Connection6).matcher(content);
            else
                m = Pattern.compile(Connection4).matcher(content);
            while (m.find()) {
                Socket socket = (is6) ? new Socket6(m.group()) : new Socket4(
                        m.group());
                ContentValues values = new ContentValues();
                values.put(Database.DatabaseSchema.Sockets.COLUMN_NAME_DATE,
                        time);
                values.put(Database.DatabaseSchema.Sockets.COLUMN_NAME_LIP,
                        socket.getLocalIP());
                values.put(Database.DatabaseSchema.Sockets.COLUMN_NAME_LPORT,
                        socket.getLocalPort());
                values.put(Database.DatabaseSchema.Sockets.COLUMN_NAME_RIP,
                        socket.getRemoteIP());
                values.put(Database.DatabaseSchema.Sockets.COLUMN_NAME_RPORT,
                        socket.getRemotePort());
                values.put(Database.DatabaseSchema.Sockets.COLUMN_NAME_STATUS,
                        socket.getStatus());
                values.put(Database.DatabaseSchema.Sockets.COLUMN_NAME_UID,
                        socket.getUid());
                values.put(Database.DatabaseSchema.Sockets.COLUMN_NAME_TYPE,
                        type);
                db.insert(Database.DatabaseSchema.Sockets.TABLE_NAME, null,
                        values);
            }
        }
    }

    // class representing an IPv6/v4 socket. Must be extended to implement
    // formatIP
    private abstract static class Socket {
        ...
        private enum states {
            UNKNOWN(0), ESTABLISHED(1), SYN_SENT(2), SYN_RECV(3), FIN_WAIT1(4), FIN_WAIT2(
                    5), TIME_WAIT(6), CLOSE(7), CLOSE_WAIT(8), LAST_ACK(9), LISTEN(
                    10), CLOSING(11), NEW_SYN_RECV(12), MAX_STATES(13);
        ...
        private void setLocalSocket(String[] lSocket) {
            // must reverse the bytes first, then format appropriately
        ...
        private void setStatus(String status) {
            this.status = states.values()[Integer.parseInt(status, 16)].name()
                    .toLowerCase();
        }
    }
    ...
    private static class Socket4 extends Socket {
        ...
        public String formatIP(String[] localIP) {
            StringBuilder result = new StringBuilder().append(Integer
                    .decode("0x" + localIP[0]));
            for (int i = 1; i < localIP.length; i++) {
                result.append('.' + Integer.decode("0x" + localIP[i])
                        .toString());
            }
    ...
    private static class Socket6 extends Socket {
        ...
        // just append a dots inbeetween. IPv6 addresses are in hexadecimal by
        // nature
        public String formatIP(String[] localIP) {
