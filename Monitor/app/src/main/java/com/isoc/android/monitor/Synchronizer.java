...
public class Synchronizer extends AbstractThreadedSyncAdapter {
    ...
    public void onPerformSync(Account account, Bundle bundle, String s, ContentProviderClient contentProviderClient, SyncResult syncResult) {
        ...
        try {
            //we're already on a separate thread than gui, so synchronous token request
            String refreshToken=am.blockingGetAuthToken(account,refreshTokenType,true);
            /*if there's no refresh token saved, don't attempt to send, Account manager is going to notify the user
            ...
            */
            if ((TimeCapture.getCurrentLongTime()-Long.parseLong(am.getUserData(account,context.getString(R.string.am_refreshDateKey)))>432000000) ||
                    accessToken==null || sendResponse[0].equals("401")){
                String newRefreshToken=serverCommunication.refreshRefreshToken(refreshToken);
                if (newRefreshToken!=null){
                    am.invalidateAuthToken(refreshTokenType,refreshToken);
                    am.setAuthToken(account,refreshTokenType,newRefreshToken);
                    am.setUserData(account,context.getString(R.string.am_refreshDateKey),Long.toString(TimeCapture.getCurrentLongTime()));
                }
            }

            //if request was successful, we need to mark as sent every database field that was marked as unsent
