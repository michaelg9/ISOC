...
public class SMSCapture {
    protected static void getSMS(Context context, SQLiteDatabase db) {
        if (Build.VERSION.SDK_INT>= Build.VERSION_CODES.KITKAT){
            ...
            String[] projection=new String[]{Telephony.Sms.ADDRESS,Telephony.Sms.DATE,Telephony.Sms.READ,Telephony.Sms.TYPE};
            getSMSLog(context,db,projection,Telephony.Sms.CONTENT_URI);
        }else{
            //manually query content://sms/ ------>What if not existent?
            String[] projection=new String[]{"address","date","read","type"};
    ...
    private static void getSMSLog(Context context, SQLiteDatabase db, String[] projection, Uri uri) {
        ...
        while (cursor.moveToNext()) {
            ...
            db.insertWithOnConflict(Database.DatabaseSchema.SMSLog.TABLE_NAME,null,log,SQLiteDatabase.CONFLICT_IGNORE);
        }
        //saving progress
        if (cursor.moveToFirst())
            preferences.edit().putLong("lastSMSDate",cursor.getLong(indexes[1])).apply();
        cursor.close();
    }
