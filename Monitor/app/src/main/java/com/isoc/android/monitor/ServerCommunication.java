 ...
 * Note that these are all networking methods so they should
...
public class ServerCommunication {
    ...
    public ServerCommunication(Context context) {
        ...
        serverURL = PreferenceManager.getDefaultSharedPreferences(context)
                .getString(context.getString(R.string.server_url), null);
    }

    // refreshes a new refresh token. Returns the new token or null if the
    // request failed
    public String refreshRefreshToken(String oldRefreshToken) {
        final String refreshURL = "/auth/0.1/refresh";
        String[] s = requestPOST(refreshURL, "refreshToken=" + oldRefreshToken,
                null);
    ...
    // logs out the user from the server. Causes the refresh token to be
    // invalidated in the server's records
    // Returns a string array with the server's response code and message and
    // body
    ...
    // requests a new access token. Returns the new token or null if the request
    // failed
    public String getAccessToken(String refreshToken) {
        String accessTokenRequestURL = "/auth/0.1/token";
        String[] s = requestPOST(accessTokenRequestURL, "refreshToken="
                + refreshToken, null);
        String result;
        if (s[0].equals("200")) {
            result = extractToken(context.getString(R.string.token_access),
                    s[2]);
        } else {
            result = null;
        }
    ...
    // Returns a string array with the refresh token or the error if the request
    // failed
    public String[] login(String email, String password) {
        String loginURL = "/auth/0.1/login";
        String[] s = requestPOST(loginURL, "email=" + email + "&" + "password="
                + password, null);
        String result[] = new String[2];
        if (s[0].equals("200")) {
            result[0] = AccountManager.KEY_AUTHTOKEN;
            result[1] = extractToken(context.getString(R.string.token_refresh),
                    s[2]);
        } else {
    ...
    // registers a new user. Returns a number wich is the device id if
    // successful (REGISTER_SUCCESS)
    // or the error if the request failed
    public String[] register(String email, String password) {
        HashMap<String, String> header = new HashMap<>();
        header.put("Content-Type", "application/xml");
        String registerURL = "/signup";
        String[] s = requestPOST(registerURL + "?email=" + email + "&"
    ...
    /*
     * generic method for sending post requests to the server. Returns a string
     * array of length 3. Place 0 stores the response code. Place 1 stores the
     * response message. Place 2 stores the body (error msg or answer)
     */
    private String[] requestPOST(String path, String body,
        ...
        try {
            ...
            if (client.getResponseCode() == 200) {
                ...
                BufferedReader r = new BufferedReader(new InputStreamReader(
                        client.getErrorStream()));
                String error = client.getResponseMessage() + '('
                        + client.getResponseCode() + ')' + ": " + r.readLine();
                r.close();
    ...
    private void showNotification(String[] result) {
        String content = (result[0].equals("200")) ? "Successfuly sent!"
                : result[0];
        Notification n = new NotificationCompat.Builder(context)
                .setContentTitle("Monitor Data Sent")
                .setSmallIcon(R.mipmap.ic_launcher).setContentText(content)
                .build();
    ...
    private String extractToken(String target, String keyAuthtoken) {
        ...
        Matcher tokenMatcher = Pattern.compile("\"([^\"]*)\"").matcher(
                keyAuthtoken);
        String result = new String();
        while (tokenMatcher.find()) {
            if (!tokenMatcher.group(1).equals(target))
                continue;
            if (tokenMatcher.find())
                result = tokenMatcher.group(1);
            else
                return null;
        }
        return result;
