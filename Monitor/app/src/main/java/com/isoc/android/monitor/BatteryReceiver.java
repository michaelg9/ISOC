...
public class BatteryReceiver extends BroadcastReceiver {
    ...
    /**
     * Captures battery details, after being triggered by BatteryReceiver. Not
     * captured on intervals
     */
    private class BatteryCapture {
        public void getBatteryStats(Context context) {
            ...
            String charging = resolveChargingMode(battery.getIntExtra(
                    BatteryManager.EXTRA_PLUGGED, -1));
            values.put(Database.DatabaseSchema.Battery.COLUMN_NAME_TEMP,
                    ((float) battery.getIntExtra(
                            BatteryManager.EXTRA_TEMPERATURE, -1)) / 10);
            values.put(Database.DatabaseSchema.Battery.COLUMN_NAME_CHARGING,
                    charging);
            values.put(Database.DatabaseSchema.Battery.COLUMN_NAME_LEVEL,
                    battery.getIntExtra(BatteryManager.EXTRA_LEVEL, -1));
            values.put(Database.DatabaseSchema.Battery.COLUMN_NAME_TIME,
                    TimeCapture.getCurrentStringTime());
            SQLiteDatabase db = new Database(context).getWritableDatabase();
            db.insert(Database.DatabaseSchema.Battery.TABLE_NAME, null, values);
            db.close();
        }

        //converts the status integer to a string
